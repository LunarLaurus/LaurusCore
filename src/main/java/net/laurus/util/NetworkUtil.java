...
public class NetworkUtil {
    ...
    /**
     * Default method to map clients to a Bitmap assuming default values for thread pool size and subnet mask.
     *
     * @param ipv4Addresses A list of IPv4 addresses to validate.
     * @param functionToValidateAddressIsToBeSet A function that takes an IPv4 address and returns a boolean indicating whether the address should be marked.
     * @return A Bitmap with the addresses mapped according to the provided function.
     * @param blacklist A list passed in, where clients that respond but get mapped to, then later ignored.
     */
    public static Bitmap mapClientsToBitmap(List<IPv4Address> ipv4Addresses, Function<IPv4Address, Boolean> functionToValidateAddressIsToBeSet, @NonNull List<Integer> blacklist) {
        return mapClientsToBitmap(ipv4Addresses, functionToValidateAddressIsToBeSet, new SubnetMask("255.255.255.0"), Executors.newFixedThreadPool(10), blacklist);
    }
    
    /**
     * Maps a list of IPv4 addresses to a Bitmap using a validation function that determines if an address should be marked.
     *
     * @param ipv4Addresses A list of IPv4 addresses to validate.
     * @param validationFunction A function that takes an IPv4 address and returns a boolean indicating whether the address should be marked.
     * @param subnetMask The subnet mask to use for determining the valid range of addresses (defaults to 255.255.255.0 if not provided).
     * @param executorService Thread pool for concurrent execution.
     * @param blacklist A list passed in, where clients that respond but get mapped to, then later ignored.
     * @return A Bitmap where each bit represents whether the corresponding address is valid based on the provided function.
     */
    public static Bitmap mapClientsToBitmap(
    		@NonNull List<IPv4Address> ipv4Addresses,
            @NonNull Function<IPv4Address, Boolean> validationFunction,
            @NonNull SubnetMask subnetMask,
            @NonNull ExecutorService executorService,
            @NonNull List<Integer> blacklist) {
    ...
    /**
     * Converts an IPv4 address to an index in the Bitmap assuming the provided subnet mask.
     * 
     * @param address The IPv4 address to convert.
     * @param subnetMask The subnet mask (e.g., 255.255.255.0) used to calculate the index.
     * @return The index corresponding to the IPv4 address within the specified subnet range.
     */
    private static int ipv4ToIndex(IPv4Address address, SubnetMask subnetMask) {
        ...
        // The index is essentially the host portion of the IP address, which is the address 
     ...
     * @param timeout The timeout in milliseconds before the ping attempt is considered failed.
